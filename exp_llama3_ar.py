...
parser.add_argument("--use_flash_decoding", action="store_true")
...
# ─── EXPERIMENT METADATA ────────────────────────────────────────────────────────
exp_base = f"{args.dataset_name}_llama3.1_ar_flash={args.use_flash_decoding}"
...
# ─── MAIN LOOP ─────────────────────────────────────────────────────────────────
for length in lengths[args.start_length_id:]:
    ...
    for rec in tqdm(recs, desc=f"Len={length}"):
        ...
        for run in range(2):
            ...
            acc_tokens_per_sec.append(gen_tokens / elapsed)
