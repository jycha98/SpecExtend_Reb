...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, nodes, threshold=0.5, max_depth=10,print_time=False):
        ...
        # # test generate with draft model
        # prefix = self.tokenizer.decode(input_ids[0,-10:].tolist(), skip_special_tokens=True)
        # print(colored(f"\n{prefix}", 'yellow'), end='')
        # self.test_autoregressive_generation(out_hidden[:, -1:], next_token, past_key_values, num_tokens=100)
        # return
        ...
        while True:
            ...
            # print(colored(f'out_hidden.shape: {out_hidden.shape}','magenta'))
    ...
    @torch.no_grad()
    def test_autoregressive_generation(self, out_hidden, next_token, past_key_values, num_tokens=10):
